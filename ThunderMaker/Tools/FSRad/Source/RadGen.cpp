...
void	RadGen::go()
{
	try
	{
		if (writeOctFile() && (lightmapWidth() != 128 || lightmapHeight() != 128))
		{
			throw "You've chosen to create an OCT file for output, which requires a lightmap resolution of 128x128.";
		}
		...
		// Load up the geometry
		{
			...
			int	extIndex = inputFilename().length() - 3;
			if (extIndex < 0) extIndex = 0;

			fstl::string	ext = inputFilename().substring(extIndex);
			ext.tolower();
